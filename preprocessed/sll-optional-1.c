...
int main()
{
 ...
 while (__VERIFIER_nondet_int()) { x->next = malloc(sizeof(SLL)); x = x->next; x->next = 0; if (__VERIFIER_nondet_int()) { x->data = 2; x->opt = malloc(sizeof(SLL)); x->opt->next = 0; } else if (__VERIFIER_nondet_int()) { x->data = 1; x->opt = x; } else { x->data = 0; x->opt = x; } }
